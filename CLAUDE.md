...
**목표**: 트렌드 급등 + 랭킹 상승이 동시에 일어나는 "기획전 골든타임" 자동 감지 (강화판)

**감지 지표** (8개, 0~100점 신뢰도 점수로 종합):
1. 트렌드 급등 — 구글/네이버 전주 대비 **+20% 이상**(`TREND_SURGE_THRESHOLD`) 상승 (30점)
2. 랭킹 급등 — **5계단 이상**(`RANK_SURGE_THRESHOLD`) 상승 또는 신규 진입 (25점)
3. 할인율 급등 — 카테고리 평균 할인율 전날 대비 5%p 이상 상승 (15점)
4. 품절 비율 급증 — 카테고리 내 품절 상품 20% 이상 (10점)
5. 복수 카테고리 교차 — 동일 키워드가 상의+아우터/바지 등 2개 이상에서 동시 급등 (10점)
6. YoY 비교 — 작년 동기(`musinsa_archive`) 동일 키워드 랭킹 대비 순위 상승 (10점)
7. 할인 지속성 — `snapshot_store` 히스토리 기준 카테고리 평균 할인율이 N일 연속 상승 중인지 (최대 5점 보너스)
