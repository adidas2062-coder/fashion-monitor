...
**감지 지표** (0~100점 신뢰도 점수로 종합):
1. 랭킹 급등 — **5계단 이상**(`RANK_SURGE_THRESHOLD`) 상승 또는 신규 진입 (30점,
   카테고리 실측 비중 가중 후 35점 캡) [내부·주]
2. 내부 키워드 흐름 — 랭킹 내 키워드 포함 상품 수의 최근 2일 vs 직전 5일 추세
   (`_keyword_flow`, **-15~+20점**: 급증 +20 / 증가 +12 / 안정 +3 / 감소 -8 / 급감 -15;
   4일 미만 히스토리 또는 평균 3개 미만 표본이면 0점) [내부·주]
3. 실시간 검색어 — 무신사 실시간 검색어 TOP30 (`collectors/musinsa_keywords`) 진입/상승
   (NEW·▲3 이상 15점 / 상승 10점 / 유지 4점) [내부·주]
...
6. 품절 비율 급증 — 카테고리 내 품절 상품 20% 이상 (5점)
7. 복수 카테고리 교차 — 동일 키워드가 상의+아우터/바지 등 2개 이상에서 동시 급등 (5점)
8. YoY 비교 — 작년 동기(`musinsa_archive`) 동일 키워드 랭킹 대비 순위 상승 (5점)
